...
const BubbleGraph = (props) => {
  ...
  useEffect(() => {
    data?.forEach((dataItem) => {
      Object.keys(dataItem?.weeks)?.map((week) => {
        setNewData((prevState) => [...prevState, {...dataItem, commit: dataItem?.weeks?.[week]?.reduce((acc, item) => acc + item.c, 0), month: week, }]);
      });
    });
  }, [data]);
  ...
  const constructBubbleGraph = () => {
    ...
    const minContributions = newData.reduce((min, obj) => Math.min(min, obj.commit), Infinity);
  const maxContributions = newData.reduce((max, obj) => Math.max(max, obj.commit), -Infinity);
    ...
    svg.append('g')
      ...
      .attr("r", function (d) { console.log(d); return z(d.commit); } )
      .style("fill", function (d) { return myColor(d.login); } )
  ...
  return (
    <div className="bubble-graph"></div>
  );
};
